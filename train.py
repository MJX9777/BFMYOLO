...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Optimizer
    nbs = 64 if "MFB" not in str(Path(opt.cfg).stem) else total_batch_size # nominal batch size
    ...
    if opt.adam:
        optimizer = optim.Adam(pg0, lr=hyp['lr0'] * 0.1, betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cache-images', default=True, action='store_true', help='cache images for faster training')
    ...
    parser.add_argument('--name', default='MFC', help='save to project/name')
